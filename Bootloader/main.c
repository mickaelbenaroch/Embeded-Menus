					...
					switch (state)
					{
						case FILE_FETCH:
			              ...
			              BootImageFileError = TRUE;
    								//FillDisplay(0x00);
    								//oledPutROMString((ROM_STRING)" Card is Empty ", 0, 0);
								}
								break;				
							}
						...
						case SCREEN_UPDATE:
							{
									...
									for (i = 0; i <7; i++)
									{
										...
										{
											oledPutString((unsigned char *)&fileList[(lineOffset+i)*sizeof(searchRecord.filename)], i, 15);
										}
									}

									if (lineOffset+lineSelect < filesCounter)
									{
										oledPutROMString((ROM_STRING)">>", lineSelect, 0); 
									}
									else
									{
										lineSelect --;
